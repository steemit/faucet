...
router.get('/confirm_account', apiMiddleware(async (req) => {
  ...
  if (accounts && accounts.length > 0 && accounts.find(a => a.name === user.username)) {
    return { success: true, username: '', reservedUsername: user.username, locale: user.locale };
  }
  return { success: true, username: user.username, reservedUsername: '', query: user.metadata.query, locale: user.locale };
}));
